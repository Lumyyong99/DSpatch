...
subfolders = ['fake_patch', 'combined_images', 'visualize_results', 'combined_images_compression']  # fake_patch用于存储生成的50xPatch_per_Image个补丁
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in group_dict_list:
        ...
        success, image_squeeze_np_JPEGCompression = cv2.imencode('.jpg',image_squeeze_np,params)


        if success:
            # 恢复JPEG Compression的图片
            restored_image = cv2.imdecode(image_squeeze_np_JPEGCompression, cv2.IMREAD_COLOR)
            perturbed_blurimage_save_path = os.path.join(args.work_dir, 'combined_images_compression', f'test_epoch_{epoch}_image_{scene_image_index}.jpg')
            cv2.imwrite(perturbed_blurimage_save_path, restored_image)
        else:
