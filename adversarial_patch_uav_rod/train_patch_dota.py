...
相比于train_patch_dota2修改处理修改patch apply的部分。train_patch_dota2中“判断一张图是否能放补丁”和“补丁放置具体位置”用了两次随机选择，导致可能会出现第一次判断出来能用但是第二次找不到位置导致返回none的情况。
"""
import argparse
...
from ensemble_tools.utils0 import find_patch_positions_v3, zero_out_bounding_boxes_v2, weights_init, convert_to_le90
...
parser.add_argument('--patch_pattern', default='dota-scene', help="patch patten. Select from [dota-scene].")
...
# scene setting
if args.patch_pattern == 'dota-scene':
    ContinuousFramesImageFolder = "../../datasets/DOTA-V1.0/DOTA-plane-propersize/images"
    ContinuousFramesLabelFolder = "../../datasets/DOTA-V1.0/DOTA-plane-propersize/labelTxt"
    ...
    while i < len(PatchDataloader):
        ...
        for gen_flag in range(args.Giters):
            ...
            while len(images_list) < image_BatchSize:
                ...
                patch_position = find_patch_positions_v3(img_size=(512, 512), 
                                                         bounding_box=single_image_annotations_le90,
                                                         patch_size=rowPatch_size,
            ...
            fake_labels_batch_t = fake_labels_batch_t * class_idx
            ...
            # 对images进行resize
            fake_patch = fake_patch.view(image_BatchSize, Patch_per_Image, 3, rowPatch_size, rowPatch_size)
