...
# parser.add_argument('--iteration', default=10, type=int, help='iteration number of I-FGSM.')
...
DetectorCheckpoint = '/home/yyx/Adversarial/mmrotate-0.3.3/UAV-ROD_training/detection_model/detectors_640x360/experiment1/latest.pth'
# scene setting
...
alphas = [0.1, 0.2, 0.3, 0.4]  # alpha相当于FGSM里的epsilon步长
epsilon = 1
...
Patch_per_Image = 1  # 这里固定为1，FGSM中一张图设定为一个fake bounding box
img_size = (384, 640)
iteration_numbers = [2, 5, 10, 15]
...
for scene_bag in tqdm(group_dict_list):
    ...
    # 对每一个alpha生成perturbed image,再迭代不同的iteration_number
    for alpha in alphas:
        for iteration_number in iteration_numbers:
            ...
            cv2.imwrite(os.path.join(args.work_dir, 'perturbed_images', f'FGSM_img{img_index}_alpha{alpha}_iter{iteration_number}.jpg'), per_img_bgr)
