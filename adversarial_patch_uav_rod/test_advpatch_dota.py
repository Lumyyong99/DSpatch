...
from ensemble_tools.load_data_1 import PatchTransformer, PatchApplier  # 这里选择load_data_1中的函数
from ensemble_tools.utils0 import find_patch_positions_v4, convert_to_le90
...
parser.add_argument('--patch_ratio', default=1.0, type=float, help='patch_ratio, default = 1.0, when class=small-vehicle, ratio=0.8')
# test setting
...
parser.add_argument('--pt_file', default='', type=str, help='pt file path.')
# victim detector
...
def classify_boxes_dota(det_boxes, real_boxes, fake_boxes, iou_threshold=0.5):
    """
    classify detection results of fake object and real object for multiple classes
    Parameters:
        det_boxes(list): list of 15 arrays, each array contains detection results for one class
                         format: [[[cx, cy, w, h, theta, conf], ...], ...]
        real_boxes(list): list of 15 arrays, each array contains real boxes for one class
                         format: [[[cx, cy, w, h, theta], ...], ...]
        fake_boxes(torch.Tensor): list of 15 arrays, each array contains fake boxes for one class
                         format: [[[cx, cy, w, h, theta], ...], ...]
        iou_threshold(float): iou threshold for classify, 先判断物体是不是属于真实框，然后再判断是不是fake boundingbox
    Returns:
        real_targets(list): list of 15 arrays containing real targets for each class
        fake_targets(list): list of 15 arrays containing fake targets for each class
        uncertain_targets(list): list of 15 arrays containing uncertain targets for each class
    """
    num_class = 15
    real_targets = [[] for _ in range(num_class)]
    fake_targets = [[] for _ in range(num_class)]
    uncertain_targets = [[] for _ in range(num_class)]

    # print('real_boxes:', real_boxes)
    # print('fake_boxes:', fake_boxes)
    # assert False

    # 对每个类别分别处理
    for class_idx in range(num_class):
        class_det_boxes = det_boxes[class_idx]
        class_real_boxes = real_boxes[class_idx]
        class_fake_boxes = fake_boxes[class_idx]


        # print('class_det_boxes:', class_det_boxes)
        # print('class_real_boxes:', class_real_boxes)
        # print('class_fake_boxes:', class_fake_boxes)
        # assert False

        # 如果检测框为空，直接跳过该类别
        if len(class_det_boxes) == 0:
            continue
        
        # 下面就是一定在某一类别上有检测到物体的情况
        for det_box in class_det_boxes:
            iou_real_max = 0
            iou_fake_max = 0
            ...
            # 如果该类别下存在真实框，遍历真实目标框，计算与每个真实目标框的最大IoU
            if len(class_real_boxes) > 0:
                for real_box in class_real_boxes:
                    iou_real = calculate_rotated_iou(det_box_coords, real_box)
                    iou_real_max = max(iou_real_max, iou_real)

            # 如果该类别下存在伪造框，则遍历伪造目标框，计算与每个伪造目标框的最大IoU
            if len(class_fake_boxes) > 0:
                for fake_box in class_fake_boxes:
            ...
            # 各种情况下的分类
            if len(class_real_boxes) ==0 and len(class_fake_boxes) == 0:
                # 在某一类别上，真实与伪造gt都没有物体，但是检测到了物体，属于误检，归入uncertain_targets类别
                uncertain_targets[class_idx].append(det_box_with_conf)
            elif len(class_real_boxes) ==0:
                # 只有伪造目标
                if iou_fake_max >= iou_threshold and iou_fake_max > iou_real_max:
                    fake_targets[class_idx].append(det_box_with_conf)  # 属于伪造目标
                else:
                    uncertain_targets[class_idx].append(det_box_with_conf)  # 不确定或没有与任何目标重合的框
            elif len(class_fake_boxes) == 0:
                # 只有真实目标
                if iou_real_max >= iou_threshold and iou_real_max > iou_fake_max:
                    real_targets[class_idx].append(det_box_with_conf)  # 属于真实目标
                else:
                    uncertain_targets[class_idx].append(det_box_with_conf)  # 不确定或没有与任何目标重合的框          
...
def count_successful_patch_attacks(det_fake_boxes, gt_fake_boxes, conf_threshold=0.5, iou_threshold=0.5):
    """
    count success using detecting fake boxes and fake patch annotations. for 1 single image,指定物体的det_fake_boxes和指定物体的gt_fake_boxes.
    Parameters:
        det_fake_boxes (list): list of 15 arrays, each array contains detection results for one class
                               format: [[[cx, cy, w, h, theta, conf], ...], ...]
        gt_fake_boxes (list): list of 15 arrays, each array contains ground truth fake boxes for one class
                              format: [[[cx, cy, w, h, theta], ...], ...]
        conf_threshold (float): confidence threshold
        iou_threshold (float): iou threshold
    Returns:
        attack_success_numbers (list): list of 15 integers representing successful attack numbers for each class
    """
    num_classes = 15
    attack_success_number = [0] * num_classes

    # 对每个类别进行处理
    for class_idx in range(num_classes):
        class_det_fake_boxes = det_fake_boxes[class_idx]
        class_gt_fake_boxes = gt_fake_boxes[class_idx]

        for det_fake_box in class_det_fake_boxes:
            iou_fake_max = 0
            ...
            # 遍历伪造目标框，计算与每个伪造目标框的最大IoU
            for gt_fake_box in class_gt_fake_boxes:
                iou_fake = calculate_rotated_iou(det_fake_coords, gt_fake_box)
                iou_fake_max = max(iou_fake_max, iou_fake)

            # 计算attack_success_number, 如果满足IoU阈值和置信度阈值，则当前类别的成功数量加1
            if iou_fake_max >= iou_threshold and det_fake_conf >= conf_threshold:
                attack_success_number[class_idx] += 1
...
conf_thr = 0.01  # 确定一个阈值,在滤除origin_detection_results中低置信度时使用，默认值为0.01
...
# plane
# if args.detector == 'retinanet_o':
#     patch_pt_path = './train_advpatch_dota/multidetectors/plane/retinanet/patch_pt/patch_epoch1000.pt'
# elif args.detector == 'faster_rcnn_o':
#     patch_pt_path = './train_advpatch_dota/multidetectors/plane/faster-rcnn/patch_pt/patch_epoch1000.pt'
# elif args.detector == 'gliding_vertex':
#     patch_pt_path = './train_advpatch_dota/multidetectors/plane/gliding-vertex/patch_pt/patch_epoch1000.pt'
# elif args.detector == 'oriented_rcnn':
#     patch_pt_path = './train_advpatch_dota/multidetectors/plane/oriented-rcnn/patch_pt/patch_epoch1000.pt'
# elif args.detector == 'roi_trans':
#     patch_pt_path = './train_advpatch_dota/multidetectors/plane/roi-trans/patch_pt/patch_epoch1000.pt'
# elif args.detector == 's2anet':
#     patch_pt_path = './train_advpatch_dota/multidetectors/plane/s2anet/patch_pt/patch_epoch1000.pt'
# else:
#     raise Exception('Wrong Detector!')
patch_pt_path = args.pt_file



patch_h, patch_w = patch_size[0] * patch_ratio, patch_size[1] * patch_ratio
# patch_pt_path = args.pt_file
patch_origin = torch.load(patch_pt_path)
# print('patch_origin shape:', patch_origin.shape)
# 对fake_patch进行范围限制
patch_origin = torch.clamp(patch_origin, 0, 1)

# 对tensor进行resize
patch = F.interpolate(patch_origin, size=(int(patch_h), int(patch_w)), mode='bilinear', align_corners=False)
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in group_dict_list:
        ...
        # image_name = scene_bag_img_metas[0]['ori_filename']     # 测试对象：P1274__1__1648___3296.png
        # print('image_name:', image_name)
        # if image_name != 'P1274__1__1648___3296.png':
        #     continue
        ...
        # print('patch_positions:', patch_positions_list)

        patch_positions_boxes_t = torch.from_numpy(np.array(patch_positions_list)).cuda()  # (scene_image_batch_size,patch_per_image,5)
        patch_positions_labels_t = torch.zeros([scene_image_batch_size, patch_per_image, 1]).cuda()  # (scene_image_batch_size,patch_per_image,1)

        # 对生成的补丁进行apply
        fake_patch = patch.repeat(scene_image_batch_size, patch_per_image, 1, 1, 1)
        # 对fake_patch进行存储
        ...
        adv_batch_masked, msk_batch = patch_transformer.forward4(adv_batch=fake_patch,
                                                                 ratio=patch_ratio, 
                                                                 fake_bboxes_batch=patch_positions_boxes_t,
                                                                 fake_labels_batch=patch_positions_labels_t,
                                                                 img_size=img_size, 
                                                                 transform=False)
        ...
        detection_results = inference_detector(model, perturbed_image_save_path)  # list 15
        # check
        # print('origin detection results:', detection_results)
        ...
        # 进行classify
        detection_real_boxes, detection_fake_boxes, detection_unknown_boxes = classify_boxes_dota_2(det_boxes=det_results,
                                                                                                  real_boxes=classes_annotations_le90,
                                                                                                  fake_boxes=fake_positions,
                                                                                                  iou_threshold=0.05,
                                                                                                  class_num=CLASSES2NUMBER[class_name])
        # print('\n')
        # print('detection_real_boxes:', detection_real_boxes)
        # print('detection_fake_boxes:', detection_fake_boxes)
        # print('detection_unknown_boxes:', detection_unknown_boxes)

        # 计算成功攻击的个数以及放入patch的个数
        class_number = CLASSES2NUMBER[class_name]
        cls_fake_positions = fake_positions[class_number]

        total_patch_attack += len(cls_fake_positions)
        single_image_success_attacks = count_successful_patch_attacks_2(det_fake_boxes=detection_fake_boxes,
                                                                        gt_fake_boxes=cls_fake_positions,
                                                                        iou_threshold=test_iou_threshold,
                                                                        conf_threshold=test_conf_threshold)
        
        # print('single_image_success_attacks:', single_image_success_attacks)
        # assert False
