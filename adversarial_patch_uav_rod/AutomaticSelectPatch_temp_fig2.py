...
def test_one_image_iharbour():
    ...
    """
    # 新建临时存储文件夹
    temp_work_dir = './vis_results_DSAP/select_diverse_stealthy_patches/fig2'
    if not os.path.exists(temp_work_dir):
        os.makedirs(temp_work_dir)
        os.chmod(temp_work_dir, stat.S_IRWXU | stat.S_IRWXG | stat.S_IRWXO)
    else:
    ...
    # 初始化
    original_image_path = '../../datasets/iharbour_dataset_2/road_scene_1_640x320/images/frame_00497.jpg'
    ...
    # load position
    positions = [
        {'center': (134, 158), 'angle': 30},
        {'center': (351, 49), 'angle': 90}
        ]
    patch_number = len(positions)    # 一张图上放置多少张补丁
    ratio = 1.0                    # 补丁放缩尺寸，默认为1
