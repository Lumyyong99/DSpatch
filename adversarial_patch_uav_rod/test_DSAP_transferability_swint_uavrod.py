...
warnings.filterwarnings("ignore", category=UserWarning, message="DeprecationWarning: anchor_generator is deprecated")
...
parser.add_argument('--victim_detector', default='retinanet_o', help='victim detector configs and checkpoints')
parser.add_argument('--source_detector', default='retinanet_o', help='target detector, on which patches are generated')
# parser.add_argument('--netG_path',default='', help='generator path')
...
# ResNet-50
res50_detectors_root = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/detection_model/detectors_640x360'
swint_detectors_root = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/detection_model/detectors_640x360_swint'
if args.victim_detector == 'rotated_retinanet':
    detector_name = 'rotated_retinanet'
    DetectorCfgSource = os.path.join(res50_detectors_root, detector_name, 'rotated_retinanet_obb_r50_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(res50_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'rotated_faster_rcnn':
    detector_name = 'faster_rcnn_o'
    DetectorCfgSource = os.path.join(res50_detectors_root, detector_name, 'rotated_faster_rcnn_r50_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(res50_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'gliding_vertex':
    detector_name = 'gliding_vertex'
    DetectorCfgSource = os.path.join(res50_detectors_root, detector_name, 'gliding_vertex_r50_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(res50_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'oriented_rcnn':
    detector_name = 'oriented_rcnn'
    DetectorCfgSource = os.path.join(res50_detectors_root, detector_name, 'oriented_rcnn_r50_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(res50_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'roi_trans':
    detector_name = 'roi_trans'
    DetectorCfgSource = os.path.join(res50_detectors_root, detector_name, 'roi_trans_r50_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(res50_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 's2anet':
    detector_name = 's2anet'
    DetectorCfgSource = os.path.join(res50_detectors_root, detector_name, 's2anet_r50_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(res50_detectors_root, detector_name, 'latest.pth')
# Swin-Tiny
elif args.victim_detector == 'retinanet_swint':
    detector_name = 'retinanet'
    DetectorCfgSource = os.path.join(swint_detectors_root, detector_name, 'rotated_retinanet_obb_swint_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(swint_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'faster_rcnn_swint':
    detector_name = 'fasterrcnn'
    DetectorCfgSource = os.path.join(swint_detectors_root, detector_name, 'rotated_faster_rcnn_swint_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(swint_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'gliding_vertex_swint':
    detector_name = 'glidingvertex'
    DetectorCfgSource = os.path.join(swint_detectors_root, detector_name, 'gliding_vertex_swint_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(swint_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'oriented_rcnn_swint':
    detector_name = 'orientedrcnn'
    DetectorCfgSource = os.path.join(swint_detectors_root, detector_name, 'oriented_rcnn_swin_tiny_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(swint_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 'roi_trans_swint':
    detector_name = 'roitrans'
    DetectorCfgSource = os.path.join(swint_detectors_root, detector_name, 'roi_trans_swin_tiny_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(swint_detectors_root, detector_name, 'latest.pth')
elif args.victim_detector == 's2anet_swint':
    detector_name = 's2anet'
    DetectorCfgSource = os.path.join(swint_detectors_root, detector_name, 's2anet_swint_fpn_1x_uavrod_le90.py')
    DetectorCheckpoint = os.path.join(swint_detectors_root, detector_name, 'latest.pth')
else:
...
# load patch generator network
generators_root = '/home/Adversarial/mmrotate-0.3.3/adversarial_patch_uav_rod/train_DSAP_uavrod/multidetectors'
# ResNet-50
print(args.source_detector)
if args.source_detector == 'rotated_retinanet':
    source_detector_name = 'retinanet_r50'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 'rotated_faster_rcnn':
    source_detector_name = 'faster_rcnn_r50'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 'gliding_vertex':
    source_detector_name = 'gliding_vertex_r50'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 'oriented_rcnn':
    source_detector_name = 'oriented_rcnn_r50'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 'roi_trans':
    source_detector_name = 'roitrans_r50'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 's2anet':
    source_detector_name = 's2anet_r50'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
# Swin-Tiny
elif args.source_detector == 'retinanet_swint':
    source_detector_name = 'retinanet_swint'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 'faster_rcnn_swint':
    source_detector_name = 'faster_rcnn_swint'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 'gliding_vertex_swint':
    source_detector_name = 'gliding_vertex_swint'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 'oriented_rcnn_swint':
    source_detector_name = 'oriented_rcnn_swint_2'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch800.pth')
elif args.source_detector == 'roi_trans_swint':
    source_detector_name = 'roi_trans_swint'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
elif args.source_detector == 's2anet_swint':
    source_detector_name = 's2anet_swint'
    netG_path = os.path.join(generators_root, source_detector_name, 'checkpoints/netG_epoch1200.pth')
else:
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in tqdm(group_dict_list):
        ...
        # print(single_image_detection_result)
        # assert False
