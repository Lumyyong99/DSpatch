...
subfolders = ['fake_patch', 'combined_images', 'visualize_results', 'combined_images_meanblur']  # fake_patch用于存储生成的50xPatch_per_Image个补丁
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in group_dict_list:
        ...
        # 进行mean blur
        image_squeeze_np_meanblur = cv2.blur(image_squeeze_np,(3,3),borderType=cv2.BORDER_CONSTANT)
        ...
        # 存储mean blur的图片
        perturbed_blurimage_save_path = os.path.join(args.work_dir, 'combined_images_meanblur', f'test_epoch_{epoch}_image_{scene_image_index}.jpg')
        cv2.imwrite(perturbed_blurimage_save_path, image_squeeze_np_meanblur)
        ...
        # print('total_success_attack=',total_success_attacks,end='\r')
