...
2025.2.19
增加gaussian blur的防御手段,属于input-level defence
"""
import argparse
...
if victim_detector == 'retinanet_o':
    ...
    DetectorCfgSource = '/home/yyx/Adversarial/mmrotate-0.3.3/UAV-ROD_models/configs/rotated_retinanet/rotated_retinanet_obb_r50_fpn_1x_uavrod_le90.py'
    DetectorCheckpoint = '/home/yyx/Adversarial/mmrotate-0.3.3/UAV-ROD_models/detection_model/detectors_640x360/rotated_retinanet_finetune/latest.pth'
else:
...
subfolders = ['fake_patch', 'combined_images', 'visualize_results', 'combined_images_blur']  # fake_patch用于存储生成的50xPatch_per_Image个补丁
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in group_dict_list:
        ...
        # 进行gaussian blur
        image_squeeze_np_gaussinblur = cv2.GaussianBlur(image_squeeze_np,(3,3),1.5)
        ...
        # 存储gaussian blur的图片
        perturbed_blurimage_save_path = os.path.join(args.work_dir, 'combined_images_blur', f'test_epoch_{epoch}_image_{scene_image_index}.jpg')
        cv2.imwrite(perturbed_blurimage_save_path, image_squeeze_np_gaussinblur)
