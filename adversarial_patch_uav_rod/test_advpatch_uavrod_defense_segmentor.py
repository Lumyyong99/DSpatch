...
log 2025.2.22
使用segmentor对图像进行预处理，测试segmentor对补丁的防御性能
"""
import argparse
...
from DefenceSegmentor import UNet
...
def load_checkpoint(model, checkpoint_path, device='cuda'):
    ...
    model.load_state_dict(checkpoint['model_state_dict'])  # 仅加载权重参数
    model = model.to(device)
    model.eval()  # 设置评估模式
    return model


def segmentor_inference(model, img_path, device, save_path, img_size=(640, 360)):
    # 预处理
    ...
    img_tensor = transforms.Compose([
        ...
        transforms.Normalize(mean=[110.928 / 255.0, 107.967 / 255.0, 108.969 / 255.0], std=[47.737 / 255.0, 48.588 / 255.0, 48.115 / 255.0])        # 处理过程中用RGB
...
if victim_detector == 'retinanet_o':
    DetectorCfgSource = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/configs/rotated_retinanet_maskfinetune/rotated_retinanet_obb_r50_fpn_1x_uavrod_le90.py'
    DetectorCheckpoint = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/detection_model/detectors_640x360/rotated_retinanet_maskaugment/latest.pth'

elif victim_detector == 'faster_rcnn_o':
    DetectorCfgSource = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/configs/rotated_faster_rcnn_maskfinetune/rotated_faster_rcnn_r50_fpn_1x_uavrod_le90.py'
    DetectorCheckpoint = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/detection_model/detectors_640x360/faster_rcnn_o-maskaugment/latest.pth'

elif victim_detector == 'gliding_vertex':
    DetectorCfgSource = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/configs/gliding_vertex_maskfinetune/gliding_vertex_r50_fpn_1x_uavrod_le90.py'
    DetectorCheckpoint = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/detection_model/detectors_640x360/gliding_vertex-maskaugment/latest.pth'
...
# elif victim_detector == 'retinanet_o_defence':
...
#     DetectorCheckpoint = '/home/Adversarial/mmrotate-0.3.3/UAV-ROD_models/detection_model/detectors_640x360/rotated_retinanet_finetune/latest.pth'
else:
...
# segmentor_checkpoint = './train_segmentor/segmentor_advpatch/segmentor_retinanet/segmentor_checkpoints/epoch_40_model.pth'
if victim_detector == 'retinanet_o':
    segmentor_checkpoint = './segmentor_training_advpatch_retinanet/segmentor_checkpoints/epoch_10_model.pth'
elif victim_detector == 'faster_rcnn_o':
    segmentor_checkpoint = './segmentor_training_advpatch_faster-rcnn/segmentor_checkpoints/epoch_10_model.pth'
elif victim_detector == 'gliding_vertex':
    segmentor_checkpoint = './segmentor_training_advpatch_gliding-vertex/segmentor_checkpoints/epoch_10_model.pth'
else:
    print('wrong detector!')
segmentor = load_checkpoint(segmentor, segmentor_checkpoint, device)
finish_time_2 = time.time()
print(f'Load segmentor in {finish_time_2 - start_time_2} seconds.')
...
subfolders = ['fake_patch', 'combined_images', 'visualize_results', 'segment_combined_images']  # fake_patch用于存储生成的50xPatch_per_Image个补丁
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in group_dict_list:
        ...
        image_squeeze_np_crop = image_squeeze_np[:360, :, :]    # (360, 640, 3), BGR
        perturbed_image_save_path = os.path.join(args.work_dir, 'combined_images', f'test_epoch_{epoch}_image_{scene_image_index}.jpg')
        cv2.imwrite(perturbed_image_save_path, image_squeeze_np_crop)

        # 使用segmentor处理添加了advpatch的图片
        segmented_save_path = os.path.join(args.work_dir, 'segment_combined_images', f'test_epoch_{epoch}_image_{scene_image_index}.jpg')
        segmentor_inference(segmentor, perturbed_image_save_path, device, segmented_save_path, img_size=(640, 360))

        # 对抗效果定量检测
        detection_results = inference_detector(model, segmented_save_path)
