...
log 2025.1.7
训练非方形补丁，对UAV-ROD数据集进行训练，不用于iharbour dataset
log 2025.2.8
为室内实验做准备，针对uavrod数据集优化出能够在物理世界中使用的advpatch，具体方式为添加对比度、亮度、尺寸等的变换
log 2025 3.3
室外实验测试，修改args.patch_pattern部分，可以从road或grass中选择
"""
import argparse
...
from ensemble_tools.utils0 import find_patch_positions_v3, find_patch_positions_v4, zero_out_bounding_boxes_v2, weights_init, convert_to_le90
...
parser.add_argument('--patch_pattern', default='road', help="patch patten. Select from ['road', ''grass].")
...
parser.add_argument('--epochs', default=1200, type=int, help='total epoch number for training process')
...
parser.add_argument('--img_format', default='jpg', type=str, help='choose a image format, from [png, jpg]. Keep consistent with training dataset')
...
### -----------------------------------------------------------    Setting     ---------------------------------------------------------------------- ###
# patch setting
patch_size = [32, 64]   # [patch_h, patch_w]
...
if args.patch_pattern == 'road':
    GAN_dataset = '/home/Adversarial/datasets/iharbour_dataset_2/RoadSnip'
elif args.patch_pattern == 'grass':
    GAN_dataset = '/home/Adversarial/datasets/iharbour_dataset_2/grassnip'
else:
...
ContinuousFramesImageFolder = "/home/Adversarial/datasets/iharbour_dataset_2/road_scene_1_640x320/images"
ContinuousFramesLabelFolder = "/home/Adversarial/datasets/iharbour_dataset_2/road_scene_1_640x320/txt_annotations"
...
PatchNormalizer = transforms.Compose(
    [
        transforms.Resize((patch_h, patch_w)),
...
GAN_input = torch.FloatTensor(GAN_training_batchsize, 3, patch_h, patch_w)
...
for epoch in range(n_epochs):
    ...
    while i < len(PatchDataloader):
        ...
        for gen_flag in range(args.Giters):
            ...
            # 对images进行resize
            fake_patch = fake_patch.view(image_BatchSize, Patch_per_Image, 3, patch_h, patch_w)
