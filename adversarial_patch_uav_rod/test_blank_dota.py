...
Test for dota blank
log 2025.1.27
dota数据集下新的test_blank中只对random类型的补丁进行测试
log 2025.2.2
之前的只能测试单一种类补丁（class_label=0），修改后测试不同种类的补丁
"""
import argparse
...
parser.add_argument('--detector', default='retinanet_o', help='victim detector')
...
subfolders = ['combined_images', 'visualize_results']  # fake_patch用于存储生成的50xPatch_per_Image个补丁
...
with open(experimental_settings_file_path, 'w') as f:
    for key, value in args_dict.items():
        f.write(f'{key}={value}\n')
    # victim detector
    f.write('Victim detector:' + detector + '\n')
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in group_dict_list:
        ...
        scene_image_index += 1
        scene_bag_image = scene_bag['Images_t']  # (1, 3, 512, 512), BGR 格式, 范围(0,1), 仅场景中的一张图
        ...
        # 为每张图生成补丁，保证这里生成的补丁范围为-1~1，与gan网络生成的对抗性补丁初始值保持一致
        min_value = -1.0
        max_value = 1.0
        generated_patch = min_value + (max_value - min_value) * torch.rand(patch_per_image, 3, patch_h, patch_w)
        generated_patch = generated_patch.to(device)
        ...
        image_squeeze = patch_applied_image_copy[0, :, :, :]  # (3, 512, 512)
