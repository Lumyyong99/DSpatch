...
2025.2.19
增加Non-Local Means Denoising的防御手段
"""
import argparse
...
subfolders = ['fake_patch', 'combined_images', 'visualize_results', 'combined_images_NLMD']  # fake_patch用于存储生成的50xPatch_per_Image个补丁
...
for epoch in tqdm(range(test_epoch)):
    ...
    for scene_bag in group_dict_list:
        ...
        # 进行Non-local means denoising
        image_squeeze_np_NLMD = cv2.fastNlMeansDenoisingColored(image_squeeze_np, 3, 3, 7, 21)
        '''
        ...
        # 存储mnon-Local means denoising的图片
        perturbed_blurimage_save_path = os.path.join(args.work_dir, 'combined_images_NLMD', f'test_epoch_{epoch}_image_{scene_image_index}.jpg')
        cv2.imwrite(perturbed_blurimage_save_path, image_squeeze_np_NLMD)


        detection_results = inference_detector(model, perturbed_blurimage_save_path)
